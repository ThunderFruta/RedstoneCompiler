...
fn BuildOneDerivedEscapeRequest(
    ...
    let (RequestId, Start, OrderedIngresses, RejectedStates, FixedPrefix, AllowedNodes) = Request;
    let Ingresses: HashSet<Position> = OrderedIngresses.into_iter().collect();
    ...
    let mut ClaimsByState: HashMap<(Position, Direction), Arc<EscapeClaimNode>> =
        HashMap::from([(StartState, PrefixClaims.expect("escape start claim"))]);
    ...
    while let Some(Reverse((Cost, Current, PriorDirection))) = Frontier.pop() {
        ...
        if Ingresses.contains(&Current) && !RejectedStateSet.contains(&CurrentState) {
...
pub(crate) fn BuildDerivedEscapeStatePathsWithDeadline(
    ...
    let Adjacency: HashMap<Position, Vec<Position>> = AdjacencyValues
        ...
        .collect();
    let CompleteExpansionUpperBounds: Vec<usize> = Requests
        .iter()
        .map(
            |(_RequestId, _Start, _Ingresses, _RejectedStates, _FixedPrefix, AllowedNodes)| {
                let AllowedNodeSet: HashSet<Position> = AllowedNodes.iter().copied().collect();
                1usize.saturating_add(
                    Adjacency
                        .iter()
                        .map(|(PositionValue, Neighbors)| {
                            if !AllowedNodeSet.contains(PositionValue) {
                                return 0;
                            }
                            Neighbors
                                .iter()
                                .filter(|Next| AllowedNodeSet.contains(Next))
                                .count()
                        })
                        .sum::<usize>(),
                )
        ...
        .collect();
    let CompleteBatchUpperBound = CompleteExpansionUpperBounds
        .iter()
        ...
        if CanParallelizeCompleteBatch {
            let Outcomes: Vec<(EscapeRequestResult, bool, bool)> =
                RoutingThreadPool().install(|| {
                    Requests
                        .into_par_iter()
                        .zip(CompleteExpansionUpperBounds.into_par_iter())
                        .map(|(Request, RequestExpansionLimit)| {
                            BuildOneDerivedEscapeRequest(
                                &Adjacency,
                                Request,
                                BendPenalty,
                                RequestExpansionLimit,
                                Deadline.clone(),
                            )
                        })
                        .collect()
