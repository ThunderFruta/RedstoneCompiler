"""Backend-neutral physical fixtures consumed by MCHPRS and Fabric."""
...
def BuildPhysicalFixture(
    *,
    ...
    if len({Value["Name"] for Value in Inputs}) != len(Inputs):
        raise ValueError("Physical fixture input names must be unique")
    if len({Value["Name"] for Value in Outputs}) != len(Outputs):
        raise ValueError("Physical fixture output names must be unique")
...
def WritePhysicalFixture(
    ...
    return PhysicalFixtureArtifact(
        Path=PathValue,
