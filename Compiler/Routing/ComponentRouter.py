...
def _ComponentNetPortfolioStructuralFingerprint(
    ...
    """Identify translation-equivalent finite per-net routing problems."""
...
def BuildComponentRoutingProblem(
    *,
    ...
    # The routed component owns only nets selected by its bounded channel:
    # internal inter-cluster nets plus an explicit authoritative cut. Merely
    # touching a selected cluster does not pull an unrelated boundary net
    # into the component; ordinary global routing retains that responsibility.
    IncidentSignals = {
        str(Signal)
    ...
    TopologyOwnedSignals = frozenset(
        str(Signal)
        for Signal in (
            *getattr(TopologyComponent, "InternalSignals", ()),
            *getattr(TopologyComponent, "BoundarySignals", ()),
        )
    )
    if TopologyComponent is not None:
        IncidentSignals.update(
            str(Signal)
            for Signal in (
    ...
    LocalClaims = tuple(
        ...
        if (
            ...
            or str(Claim.Signal) in TopologyOwnedSignals
        )
    )
    ImmutableClaims = tuple(
        ...
        if (
            int(getattr(Claim, "ClusterId", -1))
            not in SelectedClusterSet
            and str(Claim.Signal) not in TopologyOwnedSignals
        )
    )
...
def _BuildNetVariant(
    ...
    for Candidate in Candidates:
        CacheKey = (Signal, Candidate.Path)
        CandidateBlockers = (
    ...
    if (
        ...
        return NetVariantTopologyCache[TopologyCacheKey]
    SourceIndexes = tuple(
        Index
    ...
    *,
    ...
    def AccessCandidateBlockers(
        ...
        Result = frozenset(
            ...
            in ImmutableForeignClaimsForAccess
