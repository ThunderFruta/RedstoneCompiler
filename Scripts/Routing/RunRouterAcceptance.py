...
class AcceptanceCase:
    ...
    # The Fabric validator exhaustively probes up to 16 inputs and uses its
    # deterministic wide-vector policy above that.  Keep the legacy truth-row
    # count for historical manifests, but gate the live validator on this
    # actual protocol count.
    ValidationVectorCount: int | None = None
    ...
    @property
    def ExpectedFabricValidationVectorCount(self) -> int:
        return (
            self.ValidationVectorCount
            if self.ValidationVectorCount is not None
            else self.TruthTableRows
        )

    def ToDictionary(self) -> dict[str, object]:
        Result = asdict(self)
        Result["ExamplePath"] = str(self.ExamplePath)
        # This is protocol metadata introduced by the Fabric cutover.  Keep
        # the benchmark profile shape stable so historical baseline
        # compatibility remains governed by its explicit provenance rules.
        Result.pop("ValidationVectorCount", None)
...
def BuildRunArtifacts(RunDirectory: Path, RunName: str) -> dict[str, Path]:
    ...
    return {
        ...
        # Retain the retired path only so stale outputs are cleared and old
    ...
    *,
    ...
    if PhysicalDocument is not None:
        ...
        if not isinstance(RouterReliability, dict):
            ...
            RouterReliability = {}
        if RouterReliability.get("RunVerdict") not in {
            "ROUTED_AWAITING_FABRIC_SERVER_VALIDATION",
        ...
        if not isinstance(FabricValidation, dict):
            ...
            FabricValidation = {}
        if FabricValidation.get("Status") != "passed":
            Failures.append("Fabric server validation did not pass")
        RawFabricValidationBackend = FabricValidation.get("Backend")
        if RawFabricValidationBackend not in AuthoritativeServerBackends:
            Failures.append(
                "Fabric server backend is missing or non-authoritative"
            )
        else:
            FabricValidationBackend = str(RawFabricValidationBackend)
        LegacySimulationBackend = RunSummary.get("SimulationBackend")
        if (
            LegacySimulationBackend is not None
            and LegacySimulationBackend != "fabric-server"
        ):
            Failures.append("legacy simulation backend is not Fabric server")
        Diagnostics = FabricValidation.get("Diagnostics")
        if not isinstance(Diagnostics, dict):
            Failures.append("missing Fabric server validation diagnostics")
            Diagnostics = {}
        if Diagnostics.get("TestedVectors") != (
            Case.ExpectedFabricValidationVectorCount
        ):
            Failures.append(
                "Fabric validation vector count mismatch: "
        ...
        if not isinstance(FabricFixture, dict):
            ...
            FabricFixture = {}
        FixtureSha256 = FabricFixture.get("Sha256")
        ArtifactFixtureSha256 = BuildFileRecord(
            Artifacts["FabricFixture"]
        ).get("Sha256")
        if not isinstance(FixtureSha256, str) or (
            FixtureSha256 != ArtifactFixtureSha256
        ):
            Failures.append("Fabric fixture hash does not match validation evidence")
        ...
        if (
            ...
            and isinstance(FabricValidationBackend, str)
            and FabricValidationBackend
            and isinstance(LitematicComposition, dict)
            and isinstance(FixtureSha256, str)
            and len(FixtureSha256) == 64
        ):
            StableArtifactSha256 = {
                "FabricFixture": FixtureSha256,
...
def BuildCaseBaselineSummary(
    ...
    return {
        ...
        "FabricValidationBackend": ReferenceEvidence.get(
            "FabricValidationBackend"
        ),
        ...
        "FabricFixtureSha256": ReferenceEvidence.get(
            "FabricFixtureSha256"
        ),
...
def ValidateFirstValidCaseSummary(
    ...
    FabricFixtureSha256 = Summary.get("FabricFixtureSha256")
    if FabricFixtureSha256 is None:
        # Read-only compatibility for pre-Fabric captures. New captures must
        # use the fixture digest branch below.
    ...
    elif not isinstance(FabricFixtureSha256, str) or len(FabricFixtureSha256) != 64:
        raise ValueError(
            f"first-valid baseline case {Case.Name} has invalid FabricFixtureSha256"
        )
    for Name in (
        "PlacementFingerprint",
        "CandidateFingerprint",
    ...
    Backend = Summary.get(
        "FabricValidationBackend",
        Summary.get("SimulationBackend"),
    )
    if Backend not in (*AuthoritativeServerBackends, "fabric-server"):
        raise ValueError(
            f"first-valid baseline case {Case.Name} has invalid "
            "FabricValidationBackend"
        )
    ...
    if not isinstance(StableArtifactSha256, dict) or any(
        ...
        for Name in (
            "FabricFixture" if FabricFixtureSha256 is not None else "TruthTable",
            "EmittedDesignSemantic",
        )
    ):
        raise ValueError(
...
def ReadBaselineReference(PathValue: Path) -> dict[str, object]:
    ...
    for Case in AcceptanceCases:
        ...
        if FabricFixtureSha256 is None:
            ...
            raise ValueError(
                f"baseline case {Case.Name} has invalid FabricFixtureSha256"
            )
...
def BuildBaselineComparison(
    *,
    ...
    for Case in AcceptanceCases:
        ...
        ReferenceFabricValidationBackend = (
            ReferenceSummary.get("FabricValidationBackend")
            or ReferenceSummary.get("SimulationBackend")
        )
        CandidateFabricValidationBackend = (
            CandidateSummary.get("FabricValidationBackend")
            or CandidateSummary.get("SimulationBackend")
        )
        FabricValidationBackendCompatible = (
            isinstance(ReferenceFabricValidationBackend, str)
            and isinstance(CandidateFabricValidationBackend, str)
            and CandidateFabricValidationBackend == ReferenceFabricValidationBackend
        )
        if not FabricValidationBackendCompatible:
            for Run in CaseRuns:
                AppendRunFailure(
                    Run,
                    "Fabric validation backend differs from captured baseline",
                )
        CircuitPassed = (
            AccuracyPassed
            and FootprintPassed
            and SpeedPassed
            and FabricValidationBackendCompatible
        )
        ...
        CircuitComparisons[Case.Name] = {
            ...
            # Compatibility aliases for consumers of v1 comparison manifests.
            "SimulationBackendCompatible": (
                FabricValidationBackendCompatible
            ),
            "FabricValidationBackend": {
                "Baseline": ReferenceFabricValidationBackend,
                "Candidate": CandidateFabricValidationBackend,
            },
            "SimulationBackend": {
                "Baseline": ReferenceFabricValidationBackend,
                "Candidate": CandidateFabricValidationBackend,
