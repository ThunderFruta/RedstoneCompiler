...
`551ee72020447bd08840b398b0d19169b7502634`, was integrated as the second
parent of the ordinary no-fast-forward Router merge
`ea2e22d8f8743417691fdb25a642bb6427e8955f`. The merge's first parent is the
exact Runtime-integrated Router revision
`edb808bc3be36d0adf3351ae17364b5460db03a9`; its second parent is the exact
Joint checkpoint above, and its current tree is
`de2e8e777b3f209f8d5e7554ce9fc12faab538df`. This replaces the earlier pending
status for this checkpoint; it is now an actual integrated revision.

The sixteen-path Joint consumer preserves Router's final selected-access
reattestation and its public regression while adopting Joint's exact
current-envelope and production Runtime-authority policy. After a channel and
deck transform, the integrated path always rebuilds routing resources, the
selected-access solve, track preparation, and the causal envelope from the
transformed placement. It never reuses a retained pre-transform resource
graph. Current Router channel construction clears predecessor access evidence,
so the combined transition validator admits only a synchronized fresh solve:
an unrebound deck is `Stale/PhysicalInputMismatch`, malformed placement
transitions remain `Stale/PlacementTransitionMismatch`, and fresh incomplete or
unsatisfiable solves retain their distinct typed classifications.
...
The pre-merge candidate verification above establishes a combined commit-ready
development candidate; the Git merge and current tree now establish the
integrated checkpoint. A fresh seven-case Router acceptance run is retained at
`Output/Acceptance/Item9-ea2e22d8-20260908T2300Z/2026-09-08/Archives/20260908T224307.579944Z-ea2e22d8f874/`.
Its `AcceptanceManifest.json` records `Accepted=false`, and its `Summary.txt`
records `0 passed / 7 failed / 0 skipped`; this is acceptance evidence and
failure accounting, not a redefinition of the architecture or a capability
proof. The failure artifacts remain authoritative for this run.

This integrated checkpoint does not establish persistent workers, unified
native cancellation, N6 salvage, a successful seven-case physical acceptance
matrix, Fabric acceptance, performance acceptance, promotion readiness, a
