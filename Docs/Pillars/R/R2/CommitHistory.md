...
|---|---|---|
| `64efbe1` — Integrate reviewed Telemetry reporting repair into Joint | 2026-09-06 | Current Joint dependency merge. It consumes the reviewed Telemetry checkpoint `b8c1d4f` and retains the exact consumer record; the reporting production/test change remains Telemetry-owned. It supports the reviewed bounded fanout evidence, not R2 acceptance. |
| `c6d6a81` — Integrate complete Telemetry identity evidence into Joint | 2026-09-06 | Joint dependency merge that carries the exact Telemetry identity-evidence checkpoint. It is integration provenance, not an R2 production implementation claim. |
| `2902d1d` — Integrate Physical selected-access validation into Joint | 2026-09-06 | Joint dependency merge consuming exact Physical `f23293a`. It proves the Physical-to-Joint consumer boundary on the controlled fanout scope, not a second Physical implementation or R2 acceptance. |
| `46775f7` — Preserve selected routing envelopes during capacity preparation | 2026-09-06 | Current-branch Joint consumer checkpoint replacing non-ancestor sibling `a4c622b` with the same source/test change. Capacity preparation retains selected routing envelopes, with placement-envelope replay coverage. It does not establish a complete five-stage handoff, Telemetry provenance, or R2 acceptance. |
...
The three newest entries record dependency integration. `2902d1d` also adds
Joint-owned current-state preflight and typed-error consumer corrections. The
controlled fanout proves the bounded five-stage path, not full R2 acceptance.
