...
The Joint worktree prepared an explicit no-fast-forward, no-commit dependency
merge of Physical `f23293a18487cde6638c8b05be18ebd71afc0d30` into Joint
`46775f776b71d148fd261e86df8c6c59ec4edd10`; merge base
`2f69160a85f1ac800243698aebec4f172164b9f8`. The replacement Physical pin is
the direct child of original selected-access pin `1d9e899`; its independently
verified six-file binary patch digest is
`21cf686b16ce2081c110efdf6d4db7555317f0c82992569cddcaca5b7ff65d31`.
All five textual conflicts were resolved field by field. Joint's preparation
identities, whole selected paths, candidate envelopes, v17 default, explicit
v16 control, and coordinator-only final commitment were retained.

At the live consumer boundary, Joint rebuilds resources from the current placed
state before pre-route fabric construction and raw descriptor materialization.
The narrow preflight delegates terminal bindings and model fingerprints to
Physical's public helpers. Only explicit helper failures become
...
remains ordinary for enabled and disabled selected access. The finalization
adapter uses Physical's public `BuildPinAccessTechnologyFingerprint` helper
instead of reconstructing the identity locally.

The independent oracle uses the existing public two-NAND fanout's actual
selected terminal geometry and claims. Its unchanged control completes genuine
routing beyond the selected access legs. Four adversarial mutations cover a
moved terminal, unchanged terminal with changed face, a foreign frozen wire at
an actual selected electrical claim, and changed technology. A smallest legal
single-NAND public route separately reaches the exact raw descriptor
materializer. Post-mutation guards cover actual witness construction, catalog
enumeration, and solve boundaries; they reject forbidden regeneration rather
than relying only on a diagnostic count. Separate cases prove that injected
ordinary fabric errors are not reclassified.

Focused verification on the reviewed checkpoint:

- Integration replay, stale-consumer, regeneration, and propagation module:
  **8 passed in 1.70 s**.
- Three affected Physical access/profile suites: **63 passed in 0.50 s**.
- Existing handoff corruption, fingerprint-only rejection, and replay set:
  **69 passed in 2.24 s**.
- Structural and routing-contract schema gate: **7 passed in 2.06 s**.
- Collection: **1,551 tests**.
- Full non-scale suite: **1 failed, 1,546 passed, 4 skipped, 257 subtests in
  60.02 s**. The sole failure is the inherited pre-Telemetry v17-versus-v16
  source-provenance expectation; it is not changed in this checkpoint. Retained
  report: `Output/Pytest/20260906T133932.283110Z-P2/`.
- Python compilation and staged/unstaged whitespace checks passed. The loaded
  native extension is this worktree's
...
The historical Physical `dc95e349` and live `1d9e899` revisions share parent
`ffd8c76430eccb7f4ae1e06b4a777a4ed6ac3b9f` and identical production/test
content; their differences are documentation/history only. The original
`dc95e349` binary patch digest is
`411ba33bf1edeea565d4aafeea99be2efd01e5ea403acf090ff7a34f8cde856e`;
it is not relabeled as the digest of `1d9e899` or `f23293a`.

Independent final review closed both actionable findings and reran the focused
consumer tests. This checkpoint is commit-ready for its authorized local merge
commit, whose identity does not exist yet and must be added by the next-stage
record without replacing the tested Joint/Physical inputs above. Commit-ready
does not mean full R2 acceptance or production-promotion readiness. Telemetry,
MCHPRS/Fabric, scale acceptance, Reuse, Router integration, and `main` are
**Not-run** or out of scope for this checkpoint.

### 2026-09-06 current bounded frontier

Current Joint `64efbe13c14c8c7445cba6dd6111256ce06f9a36` contains the exact
Physical producer `f23293a` through `2902d1d` and consumes reviewed Telemetry
reporting repair `b8c1d4f` as a dependency merge. The Telemetry repair remains
Telemetry-owned; Joint added only its exact consumer record.

The retained public two-NAND fanout artifact now exercises the real sequence
`Placement -> AccessFabric -> RawAssignment -> DetailedRouting -> Compaction`
and coordinator publication. It retains selected geometry, repeaters, final
wires, claims, policy/technology identities, and zero resource conflicts. The
...
This supersedes only the older statement that no small successful five-stage
artifact existed. Historical FullAdder/RCA4/RCA8 diagnostics remain historical
failure evidence, not a fresh result on this current checkpoint. R2 Stage 1,
folded/alternate access, global capacity/repair, route-quality comparison,
live physical/MCHPRS/Fabric, scale acceptance, and promotion remain unproved
or not run.
...
## Current template-state local-routing candidate

Physical-Rules producer source from `86d1ee5f05684b7fc042d704c8b772847ecddd12`
is available to this bounded Joint candidate. `CommitRouting` now obtains local
template occupancy, keep-out, frozen-resource, and `BlockStates` semantics
through the same public Physical resource construction path used for global
