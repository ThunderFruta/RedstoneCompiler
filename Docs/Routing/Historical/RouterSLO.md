...
> [router reliability guide](../Active/RouterReliabilityGuide.md); its gates control when
> historical text conflicts.

> **Current gate (2026-07-22):** The active sequence and measured checkpoint
> are in [Benchmarks](../Testing/Benchmarks.md). There is no production fallback.
> FullAdder must pass first, RCA4 must then pass 2/2, and CLA4 must then pass
> 2/2. The older fallback and phase language below is retained as historical
> policy only.
