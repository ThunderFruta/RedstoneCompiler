...
class RouterAcceptanceHarnessTests(unittest.TestCase):
                ...
                with tempfile.TemporaryDirectory() as DirectoryValue:
                    ...
                    def Runner(**Options):
                        ...
                        if (
                            ...
                            Artifacts["TruthTable"].write_text(
                                Artifacts["TruthTable"].read_text(
                                    encoding="utf-8"
                                )
                                + "stable semantic rows, changed artifact\n",
                                encoding="utf-8",
                            )
                        return AcceptanceCommandResult(
                            0,
    ...
    def testTruthTableDigestIsCanonicalAndAnExplicitHardGate(self) -> None:
        Case = AcceptanceCases[0]
        with tempfile.TemporaryDirectory() as DirectoryValue:
            Root = Path(DirectoryValue)
            ...
            Evaluation, _Evidence = EvaluateRun(
                ...
                ExpectedSeed=0,
                DesignDigestBuilder=DigestFixture,
                TruthTableEvidenceBuilder=lambda _Path: {
                    "RowCount": 8,
                    "AllRowsPassed": True,
                    "ExpectedMatchesSimulated": True,
                    "ArithmeticResultSha256": WrongDigest,
                    "SimulationResultSha256": WrongDigest,
                },
            )
            self.assertFalse(Evaluation["Accepted"])
            self.assertTrue(any(
                "canonical arithmetic digest mismatch" in Failure
                for Failure in Evaluation["Failures"]
            ))
        ...
        with tempfile.TemporaryDirectory() as DirectoryValue:
            ...
            Mutations = (
                ...
                (
                    ...
                    "invalid TruthTableArithmeticSha256",
                ),
            )
