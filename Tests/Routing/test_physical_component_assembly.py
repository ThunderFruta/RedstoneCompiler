...
def test_boundary_pair_arc_uses_direct_compiled_local_support():
    ...
    BetaBadFactor = LocalFactor("Beta", "beta-bad", 400)
    BetaGoodFactor = LocalFactor("Beta", "beta-good", 420)
    Supports = {
        "Alpha": (
            SimpleNamespace(
                Signal="Alpha",
        ...
        "Beta": tuple(
            ...
            for Factor in (BetaBadFactor, BetaGoodFactor)
        ),
    }
    UnsupportedPair = frozenset((
        ...
        (
            "Beta",
            BuildPhysicalPortSeamContractFingerprint(BetaBadFactor),
        ),
    ))
    ...
    Arguments = dict(
        ...
        LocalAccessFactorsBySignal={
            ...
            "Beta": (BetaBadFactor, BetaGoodFactor),
        },
        ApertureFactorsBySignal={
            "Alpha": (
                Apertures[AlphaBad.ReservationFingerprint],
    ...
    PartiallyBlockedAssignments = tuple(
        IterPhysicalBoundaryPortAssignments(
            **Arguments,
            LearnedLocalSeamNoGoodClauses=(UnsupportedPair,),
        )
    )
    assert len(PartiallyBlockedAssignments) == 2

    RemainingUnsupportedPair = frozenset((
        (
            "Alpha",
            BuildPhysicalPortSeamContractFingerprint(AlphaBadFactor),
        ),
        (
            "Beta",
            BuildPhysicalPortSeamContractFingerprint(BetaGoodFactor),
        ),
    ))
    Assignments = tuple(IterPhysicalBoundaryPortAssignments(
        **Arguments,
