...
from PhysicalDesign.Rendering.SchemWriter import BlockProvenance, BuildLitematicBlockMap, BuildWireState, LoadTemplate, TemplateRepeaterPinRoles, ValidateSerializedRepeaterOrientations, WriteLitematic, _PlaceIoSigns
from Assets.Templates import LitematicTemplates


class PhysicalCellTests(unittest.TestCase):
    def testUnusedInputUsesTerminalBankFallback(self) -> None:
        Module = ModuleIR(
            Name="UnusedInput",
            Inputs=["Used", "Unused"],
            Outputs=["Result"],
            Gates=[
                Gate("InputUsed", GateKind.INPUT, ["Used"], []),
        ...
        self.assertEqual(
            set(PlacedByName),
            {"InputUsed", "InputUnused", "NandResult", "OutputResult"},
        )
        self.assertIsNotNone(PlacedByName["InputUnused"].OutputPin)
