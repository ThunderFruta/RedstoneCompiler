...
def testExplicitPlacementAccessFabricRejectsStaleResourcesMissingFrozenWire(
) -> None:
    Placement, Resources, _Domains, Solve = (
        _SelectedStraightAccessFixture()
    )
    ...
    ForeignPosition = min(
        Position
        for Position in SelectedSource.Claims.ElectricalCells
        if Position not in Resources.ResourceGraph.ElectricalBlocks
    )
    FrozenNetWires = {"Foreign": (ForeignPosition,)}
    CurrentPlaced = replace(
        Placement.Placed,
    ...
    assert ForeignPosition in SelectedSource.Claims.ElectricalCells
    assert ForeignPosition not in Resources.ResourceGraph.ElectricalBlocks
    with pytest.raises(ValueError):
        BuildPlacedPinAccessModelFingerprint(
            CurrentPlaced.PlacedGates,
            ResourceGraph=Resources.ResourceGraph,
