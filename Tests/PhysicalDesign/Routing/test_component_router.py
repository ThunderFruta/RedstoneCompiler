...
from PhysicalDesign.Routing.Regions.Interfaces.Fabric import ApplyRoutedComponentGlobalProfiles, AugmentComponentRoutingFabric, BuildCoalescedComponentAccessCandidates, CoalesceOwnedSignalAccessDomains, BuildClosedComponentInterface, BridgeDisconnectedOwnedSignalFabric, BuildClaimsAwareComponentFabricSubtree, BuildComponentRoutingFabric, BuildComponentFabricAdjacency, FilterExternalSourcePoweredSeamCandidateDomains, PruneDominatedComponentAccessCandidates, SelectClosedComponentOwnedTerminalPairs, SelectComponentIncidentSignals
from PhysicalDesign.Routing.Regions.Interfaces.Feedthroughs import BuildDeclaredComponentFeedthroughDomains
from PhysicalDesign.Routing.Regions.Interfaces.Problem import BuildComponentRoutingProblem
from PhysicalDesign.Routing.Regions.Domains import BuildComponentForeignTransitDomains, FindCompleteComponentNetUnsatSubset
from PhysicalDesign.Routing.Regions.Planning.Portfolios import BuildCompleteOpposingNetAccessRowContext, BuildCompleteOpposingNetAccessContractDomain, BuildOpposingNetEffectiveAccessSignature, BuildExactComponentPortRealizabilityContext, BuildExactComponentPortRealizabilityFingerprint, ClearStructuralPortRealizabilityCache, EvaluateCachedCompleteOpposingNetAccessPair, EvaluateCompleteOpposingNetAccessPair, EvaluateCompleteOpposingNetAccessContractRow, EvaluateExactComponentPortRealizability
from PhysicalDesign.Routing.Regions.Solving.Solver import MaterializeRoutedComponentTemplate, PreserveRoutedComponentForeignEscapes, SolveComponentRoutingProblem, ValidateRoutedComponentHandoff
from PhysicalDesign.Routing.Regions.Symbolic.SymbolicState import PrepareComponentSymbolicNetStateContext
from PhysicalDesign.Routing.Regions.Solving.DynamicSolver import SolveComponentRoutingProblemDynamic
from PhysicalDesign.Routing.Regions.Symbolic.SymbolicWorkers import CompilePreparedComponentSymbolicNetStates
from PhysicalDesign.Routing.Regions.Planning.NetPlanning import _BuildCanonicalAccessCombinationKey, _BuildNetVariant
from PhysicalDesign.Routing.Regions.Interfaces.Fabric import _PlanTreeRepeaters
from PhysicalDesign.Routing.Regions.Solving.LegacySolver import _SolveComponentRoutingProblemLegacy
from PhysicalDesign.Interfaces.PhysicalClaims import ComponentClaimsCompatibleForOwners, ComponentClaimsConflict
...
def test_tree_repeater_dp_handles_deep_linear_fabric_without_recursion():
    Nodes = tuple((Index, 7, 0) for Index in range(1_200))
    Edges = frozenset(
        (First, Second)
    ...
    assert Repeaters is not None
    assert len(Repeaters) >= 70
...
def test_bulk_opposing_pair_reuses_precomputed_current_contract_domain(
    ...
    monkeypatch.setattr(
        "PhysicalDesign.Routing.Regions.Planning.Portfolios."
    ...
    for Name in (
        ...
        monkeypatch.setattr(
            "PhysicalDesign.Routing.Regions.Planning.Portfolios." + Name,
...
def test_component_access_coalescing_breaks_duplicate_trunk_ties_stably():
    class HorizontalResourceGraph:
        Technology = SimpleNamespace(
            NeighborPositions=lambda Position: tuple(
                (
                    Position[0] + Delta[0],
                    Position[1] + Delta[1],
                    Position[2] + Delta[2],
    ...
    Trunk = _Candidate(((0, 1, 0), (0, 1, 1), (0, 1, 2)))
    Independent = _Candidate(((2, 1, 0), (2, 1, 1), (2, 1, 2)))

    First = BuildCoalescedComponentAccessCandidates(
        Independent,
        (Trunk, Trunk),
        ResourceGraph=HorizontalResourceGraph(),
        ExistingNodes=frozenset(Trunk.Path),
    )
    Second = BuildCoalescedComponentAccessCandidates(
        Independent,
        (Trunk, Trunk),
        ResourceGraph=HorizontalResourceGraph(),
        ExistingNodes=frozenset(Trunk.Path),
...
def test_external_source_seam_filters_disconnected_and_conflicting_access():
    ...
    assert FilterExternalSourcePoweredSeamCandidateDomains(
        ...
        LocalPath,
        CandidateEligibilityDiagnostics=EligibilityDiagnostics,
    ) == ((Legal,),)
    assert EligibilityDiagnostics["DisconnectedFabricSubtreeCount"] == 1
    assert EligibilityDiagnostics["FeasibleCount"] == 1
    assert EligibilityDiagnostics["SelfClaimConflictCount"] == 1
    assert EligibilityDiagnostics["SelfClaimConflictResourceCounts"] == {
        "Air": 1,
    }
    assert EligibilityDiagnostics["SelfClaimConflictResourceSamples"] == [
        {
            "Kind": "Air",
            "RelativePosition": [-3, 0, 0],
        },
    ]
    ...
    EligibilityCache = {}
    EligibilityCacheStatistics = {}
    CachedEligibilityKeywords = {
        "CandidateEligibilityCache": EligibilityCache,
        "CandidateEligibilityCacheStatistics": EligibilityCacheStatistics,
    }
    assert FilterExternalSourcePoweredSeamCandidateDomains(
        Problem,
        "Alpha",
        (Domain,),
        ((Legal, ConnectedButConflicting),),
        LocalPath,
        **CachedEligibilityKeywords,
    ) == ((Legal,),)
    InitialEligibilityStores = EligibilityCacheStatistics["StoreCount"]
    assert FilterExternalSourcePoweredSeamCandidateDomains(
        Problem,
        "Alpha",
        (Domain,),
    ...
    assert EligibilityCacheStatistics["HitCount"] == 2
    assert EligibilityCacheStatistics["StoreCount"] == InitialEligibilityStores
    assert FilterExternalSourcePoweredSeamCandidateDomains(
        Problem,
        "Alpha",
        (Domain,),
...
def test_physical_component_owns_all_selected_cluster_gate_terminals():
    Channel = _Channel(
        ((0, 7, 0), (1, 7, 0), (2, 7, 0)),
    )
    Request = SimpleNamespace(
    ...
    Placed = SimpleNamespace(
        ...
        PackedClusters=(
            SimpleNamespace(
                ClusterId=0,
                MemberNands=("Producer",),
            ),
            SimpleNamespace(
                ClusterId=1,
                MemberNands=("FirstConsumer", "SecondConsumer"),
            ),
        ),
        PlacedGates=(
            SimpleNamespace(
    ...
    assert SelectClosedComponentOwnedTerminalPairs(
        Placed,
        {"Alpha": Profile},
    ) == frozenset((
        ("Alpha", (0, 7, 0)),
...
def test_disconnected_owned_fabric_bridge_skips_self_conflicting_staircase():
    Start = (0, 1, 0)
    Target = (0, 3, 0)
    Fabric = ComponentRoutingFabric(
        FabricFingerprint="disconnected-vertical-islands",
        Nodes=(Start, Target),
        Edges=(),
        IngressNodes=(Start, Target),
        TopologyKind="test-disconnected-islands",
        Complete=True,
    )
    Domains = (
        _Domain("Alpha", Start, "source", _Candidate((Start,))),
        _Domain("Alpha", Target, "target", _Candidate((Target,))),
    )
    ResourceGraph = RoutingResourceGraph(
        ActualBlocks=frozenset(),
        ElectricalBlocks=frozenset(),
        SolidBlocks=frozenset(),
        GraphVersion="claims-aware-bridge-test",
    )

    Bridged = BridgeDisconnectedOwnedSignalFabric(
        Fabric,
        Domains,
        ResourceGraph,
        ProtectedAccessNodes=frozenset((Start, Target)),
    )
    Claims = ResourceGraph.BuildRouteClaims(frozenset(Bridged.Nodes))

    assert Bridged.TopologyKind == "closed-component-bridged-forest-v1"
    assert not FindSelfClaimConflicts({"Alpha": Claims})
    assert len(Bridged.Nodes) > 3


def test_access_augmentation_removes_existing_support_conflict_node():
    Terminal = (0, 1, 0)
    UpperFabricNode = (0, 2, 0)
    SafeFabricNode = (1, 2, 0)
    Fabric = ComponentRoutingFabric(
        FabricFingerprint="existing-support-conflict",
        Nodes=(Terminal, UpperFabricNode, SafeFabricNode),
        Edges=((UpperFabricNode, SafeFabricNode),),
        IngressNodes=(SafeFabricNode,),
        TopologyKind="test-existing-fabric",
        Complete=True,
    )
    ResourceGraph = RoutingResourceGraph(
        ActualBlocks=frozenset(),
        ElectricalBlocks=frozenset(),
        SolidBlocks=frozenset(),
        GraphVersion="protected-access-existing-fabric-test",
    )

    Augmented = AugmentComponentRoutingFabric(
        Fabric,
        (Terminal,),
        ResourceGraph,
        ProtectedAccessNodes=frozenset((Terminal,)),
    )

    assert UpperFabricNode not in Augmented.Nodes
    assert Terminal in Augmented.Nodes


def test_claims_aware_fabric_subtree_skips_shorter_support_conflict():
    Start = (0, 1, 0)
    BadMiddle = (-1, 2, 0)
    Target = (0, 3, 0)
    GoodPath = (
    ...
    Edges = {
        (Start, BadMiddle),
        (BadMiddle, Target),
        *zip(GoodPath, GoodPath[1:]),
    }
    Fabric = ComponentRoutingFabric(
        FabricFingerprint="short-bad-long-good",
        Nodes=tuple(sorted({Node for Edge in Edges for Node in Edge})),
        Edges=tuple(sorted(Edges)),
        IngressNodes=(Start, Target),
        TopologyKind="test-alternative-subtrees",
        Complete=True,
    )
    ResourceGraph = RoutingResourceGraph(
        ActualBlocks=frozenset(),
        ElectricalBlocks=frozenset(),
        SolidBlocks=frozenset(),
        GraphVersion="claims-aware-subtree-test",
    )

    Subtree = BuildClaimsAwareComponentFabricSubtree(
        Fabric,
        (Start, Target),
        ResourceGraph,
    )
    ...
    Claims = ResourceGraph.BuildRouteClaims(Subtree[0])
    assert BadMiddle not in Subtree[0]
    assert not FindSelfClaimConflicts({"Alpha": Claims})
...
def test_tree_power_dp_places_repeaters_on_long_trunk():
    ...
    assert Result.Feasible
    assert Result.Template is not None
    assert len(Result.Template.Nets[0].RepeaterInputFacings) >= 2
