...
class RouterReliabilityTests(unittest.TestCase):
    ...
    def testSlowDeferredGeneratorCannotConsumeRoutingReserve(self) -> None:
        ...
        with patch(
            "Compiler.Placement.Flow.Demand.BuildPlacementGenerationPlan",
            return_value=Plan,
        ):
    ...
    def testLargeTopologyDoesNotOverrideFeedbackRoundPolicy(
        ...
        Base = self.BuildReconvergentTopologyModule()
        Padding = [
            SimpleNamespace(
                Kind="NAND",
                Inputs=(),
                Outputs=(f"Padding{Index}",),
            )
            for Index in range(72 - len(Base.Gates))
        ]
        LargeModule = SimpleNamespace(
            Gates=[*Base.Gates, *Padding],
            Inputs=Base.Inputs,
            Outputs=Base.Outputs,
        )

        Result, _SuccessfulRoutes, _DeadlineIdentities = (
            self.RunTwoPlacementFlow(NetlistModule=LargeModule)
        )

        self.assertEqual(
            Result.Policy.NegotiatedRouting.MaximumPlacementFeedbackRounds,
            1,
        )
    ...
    def testExactRowBeamCutPrioritizesRelocationBeforeDirectOnly(self) -> None:
        ...
        def PlaceBehavior(
            ...
            if CallIndex == 2:
                return SecondPlacement
            FirstGate = FirstPlacement.Placed.PlacedGates[0]
            FallbackGateValues = vars(FirstGate).copy()
            FallbackGateValues["Z"] = 1
            FallbackGateValues["MirrorX"] = True
            return replace(
                FirstPlacement,
                Placed=replace(
                    FirstPlacement.Placed,
                    PlacedGates=[
                        SimpleNamespace(**FallbackGateValues)
                    ],
                    LocalRouteClaims=(),
                ),
            )
        ...
        def RouteBehavior(X, SuccessfulRoute, _Options):
            ...
            raise RoutingStageError(
                RoutingFailure(
                    Reason=RoutingFailureReason.TrackAssignmentConflict,
                    Stage="TrackAssignment",
                    AffectedNets=("CarryOut", "Owned", "Sum0"),
                    RepairActions=("AdvancePlacementCandidate",),
                    Diagnostics={
                        "Action": "advance-placement-conflict-relocation",
                        "ConflictGraph": {
                        ...
                        "EscalationHistory": [
                            {
                                ...
                                "AffectedSignals": ["Sum0"],
                                "Diagnostics": {"Materialized": 0},
                            },
                        ],
                    },
                )
        ...
        Attempts = Result.Routed.RoutingControlEffectiveness[
            "PlacementAttempts"
        ]
        self.assertEqual(
            [Attempt["SourceGenerator"] for Attempt in Attempts],
            [
                "row-beam",
                "row-beam-conflict-relocation",
                "row-beam-direct-only",
        ...
        self.assertIs(Result.Routed, SuccessfulRoutes[0])
        Decisions = Result.Routed.RoutingControlEffectiveness[
            "PlacementGenerationDecisions"
        ]
        self.assertFalse(any(
            Decision.get("Result")
            == "prioritize-direct-only-after-exact-cut"
    ...
    def testRepeatedAccessDistinctCutRetriesSamePlacementBeforeGeneration(
        ...
        def PlaceBehavior(
            ...
            if CallIndex > 2:
                self.fail(
                    "new placement generation preempted the coordinated retry"
                )
            Placement = (
                FirstPlacement if CallIndex == 1 else SecondPlacement
            )
            CoordinatedSignals = sorted(
                Options["CoordinatedCandidateDiversificationSignals"]
            )
            Result = replace(
                Placement,
                Placed=replace(
                    ...
                    LocalRouteDiagnostics={
                        "__PlacementRelocation__": {
                            "Signals": sorted(Options["RelocationSignals"]),
                            "PrioritySignals": sorted(
                                Options["RelocationPrioritySignals"]
                            ),
                            "RequiredSignals": sorted(
                                Options["RequiredRelocationSignals"]
                            ),
                            "CoordinatedCandidateDiversificationSignals": (
                                CoordinatedSignals
                            ),
                            "CoordinatedCandidateDiversityLevel": (
                                1 if CoordinatedSignals else 0
                            ),
                        },
                    },
                ),
            )
            X = Result.Placed.PlacedGates[0].X
            PlacementCalls.append(X)
            MaterializedPlacements[X] = Result
            return Result
        ...
        def RouteBehavior(X, SuccessfulRoute, _Options):
            nonlocal RouteCallCount
            RouteCallCount += 1
            ObservedProfiles.append(dict(
                MaterializedPlacements[X]
                .Placed.LocalRouteDiagnostics[
                    "__PlacementRelocation__"
                ]
            ))
            if RouteCallCount <= 2:
            ...
            self.assertEqual(PlacementCalls, [0, 10])
            return SuccessfulRoute

        def MeasureTopology(BaseProfile, Candidate, **Options):
            Measured = MeasurePlacementTopologyDemand(
                BaseProfile,
                Candidate,
                **Options,
            )
            X = Candidate.Placed.PlacedGates[0].X
            return replace(
                Measured,
                QualifyingReconvergentCutCount=1,
                MandatoryAccessOwnershipFingerprint=f"access-{X}",
            )
        ...
        with (
            ...
            patch(
                "Compiler.Placement.Flow.Demand.MeasurePlacementTopologyDemand",
                side_effect=MeasureTopology,
            ),
        ):
            Result, SuccessfulRoutes, _DeadlineIdentities = (
                self.RunTwoPlacementFlow(
        ...
        self.assertEqual(PlacementCalls, [0, 10])
        self.assertEqual(
            [Profile["CoordinatedCandidateDiversificationSignals"]
             for Profile in ObservedProfiles],
            [
                [],
                [],
                ["A0", "B1", "Generate0", "Propagate1"],
            ],
        )
        self.assertFalse(any(
            Event.get("Status")
            == "unattempted-routing-control-profile-rebound"
            for Event in Result.Routed.RoutingControlEffectiveness[
                "JointPlacementStateEvents"
            ]
        ))
        RetryEvents = [
        ...
        self.assertEqual(len(RetryEvents), 1)
        RetryEvent = RetryEvents[0]
        self.assertEqual(
            RetryEvent["CoordinatedCandidateDiversificationSignals"],
            ["A0", "B1", "Generate0", "Propagate1"],
        )
        self.assertEqual(
            RetryEvent["EvidenceKinds"],
            ["repeated-exact-cut"],
        )
        self.assertTrue(RetryEvent["ReusedPlacedGeometry"])
        self.assertTrue(RetryEvent["ReusedRoutingResources"])
        self.assertEqual(
            RetryEvent["NextAction"],
        ...
        self.assertEqual(
            Attempts[1]["PlacementFingerprint"],
            Attempts[2]["PlacementFingerprint"],
        )
        EligibleDecisions = [
            Decision
            for Decision in Result.Routed.RoutingControlEffectiveness[
                "PlacementGenerationDecisions"
            ]
            if Decision.get(
        ...
        self.assertEqual(len(EligibleDecisions), 1)
    ...
    def testCompletedExactCutPreemptsRetainedUnpackedPlacement(self) -> None:
        ...
        ExactCut = frozenset({"A0", "B1", "Carry1", "Propagate1"})
        BroadRegenerationCover = sorted((
            *ExactCut,
            "A2",
            "B2",
            "Generate0",
        ...
        for FailureReason in (
            ...
            with self.subTest(FailureReason=FailureReason):
                RelocationInputs = []
                ...
                self.assertEqual(len(RelocationInputs), 3)
                self.assertEqual(
                    RelocationInputs[2],
                    (
                        ExactCut
                        if FailureReason
                        == RoutingFailureReason.RuntimeBudgetExceeded
                        else frozenset(BroadRegenerationCover)
                    ),
                )
                self.assertIs(Result.Routed, SuccessfulRoutes[10])
                self.assertEqual(
    ...
    def testStructuredCutRelocationPreemptsRetainedJointAndBroadStates(
        ...
        Policy = LocalFirstPhysicalDesignPolicy
        JointPacking = replace(
            Policy.NandPacking,
            EnableJointClusterOrientation=True,
            RetainedJointPlacementCandidates=2,
        )
        Plan = PlacementGenerationPlan(
            PrimaryRequests=(
                PlacementGenerationRequest(
            ...
            DeferredRequests=(
                ...
                PlacementGenerationRequest(
                    "broad-unpacked",
                    Policy.Placement.RoutingSpacing + 1,
                    replace(JointPacking, Enabled=False),
                ),
            ),
            # The fresh typed cut epoch must replace broad work even after
            # the ordinary generator budget was consumed by the primary.
            MaximumAttempts=1,
        )
        PlacementCalls = []
        ...
        def JointPlacement(
            ...
            *,
            ZOffset: int = 0,
            Rotation: int = 0,
            RetainAlternative: bool = False,
        ):
            GateValues = vars(Placement.Placed.PlacedGates[0]).copy()
            GateValues["Z"] += ZOffset
            GateValues["Rotation"] = Rotation
            GateValues["MirrorX"] = bool(CandidateIndex)
            AnchorValues = GateValues.copy()
            AnchorValues.update({
                "Name": "Anchor",
                "X": 0,
                "Z": 20,
            ...
            JointDiagnostics = {
                ...
                "SelectedScore": CandidateIndex,
                "SelectedTransforms": {},
            }
            if RetainAlternative:
                JointDiagnostics["ExactLegalRetainedStates"] = [
                    {
                        "CandidateIndex": 0,
                        "SearchScore": 0,
                        "Transforms": {},
            ...
            return replace(
                ...
                Placed=replace(
                    Placement.Placed,
                    PlacedGates=[
                        SimpleNamespace(**GateValues),
                        SimpleNamespace(**AnchorValues),
                    ],
                    LocalRouteDiagnostics={
                        "__JointClusterPlacement__": JointDiagnostics,
                    },
                ),
            )
        ...
        def PlaceBehavior(
            ...
            PlacementCalls.append({
                "CandidateIndex": Options["JointPlacementCandidateIndex"],
                "AssignmentCut": Options.get("AssignmentCut"),
                "AssignmentConstraints": Options.get(
                    "AssignmentConstraints"
                ),
                "JointOrientation": (
                    PackingPolicy.EnableJointClusterOrientation
                ),
                "MandatoryAccessPreScreenOnly": Options.get(
            ...
            if CallIndex == 1:
                return JointPlacement(
                    FirstPlacement,
                    0,
                    RetainAlternative=True,
                )
            if CallIndex == 2:
                return JointPlacement(SecondPlacement, 1)
            return JointPlacement(
                FirstPlacement,
                0,
                ZOffset=3,
        ...
        def RouteBehavior(_X, SuccessfulRoute, _Options):
            ...
            if RouteCallCount == 1:
                Failure = self.BuildHigherOrderAssignmentFailure()
                Failure.Diagnostics["ConflictGraph"]["Classification"] = (
                    "mandatory-boundary-capacity-cut"
                )
        ...
        OwnedClaim = SimpleNamespace(
            ...
            Nodes=(),
            Edges=(),
            Claims=SimpleNamespace(ResourceIds=()),
            ExactRouteSignalBlocks=0,
            ExactRouteRefreshBlocks=0,
            ExactRouteSupportBlocks=0,
        )
        with patch(
            "Compiler.Placement.Flow.Demand.BuildPlacementGenerationPlan",
            return_value=Plan,
            ...
            Result, SuccessfulRoutes, _DeadlineIdentities = (
                self.RunTwoPlacementFlow(
                    ...
                    NetlistModule=self.BuildReconvergentTopologyModule(),
                )
            )

        self.assertEqual(RouteOrder, [0, 0])
        self.assertNotIn(10, RouteOrder)
        self.assertIs(Result.Routed, SuccessfulRoutes[0])
        self.assertEqual(len(PlacementCalls), 4)
        self.assertIsNone(PlacementCalls[0]["AssignmentCut"])
        self.assertIsNone(PlacementCalls[1]["AssignmentCut"])
        RelocationCut = PlacementCalls[2]["AssignmentCut"]
        self.assertIsInstance(RelocationCut, RoutingAssignmentCut)
        RelocationConstraints = PlacementCalls[2][
            "AssignmentConstraints"
        ]
        ...
        self.assertEqual(
            RelocationConstraints.PairwiseConflictEdges,
            (("Generate0", "Propagate1"),),
        )
        self.assertEqual(
            RelocationConstraints.HigherOrderSignalSets,
            (),
        )
        self.assertTrue(PlacementCalls[2]["JointOrientation"])
        self.assertTrue(
            PlacementCalls[2]["MandatoryAccessPreScreenOnly"]
        )
        self.assertFalse(
            PlacementCalls[3]["MandatoryAccessPreScreenOnly"]
        )
        self.assertEqual(
            PlacementCalls[2]["AssignmentCut"],
            PlacementCalls[3]["AssignmentCut"],
        )
        self.assertEqual(
            RelocationCut.PairwiseConflictEdges,
            (("Generate0", "Propagate1"),),
        )
        self.assertEqual(
            RelocationCut.ConflictGraph["CapacityOnePlannerWitness"],
            {
                "Owner": "Generate0",
                "Claimant": "Propagate1",
            },
        ...
        self.assertEqual(
            [Attempt["SourceGenerator"] for Attempt in Attempts],
            ["row-beam", "row-beam-conflict-relocation"],
        )
        History = Result.Routed.RoutingControlEffectiveness[
    ...
    def testCompletedAssignmentCutSurvivesLaterBareTimeout(self) -> None:
        ...
        def PlaceBehavior(
            ...
            if CallIndex == 2:
                return SecondPlacement
            GateValues = vars(FirstPlacement.Placed.PlacedGates[0]).copy()
            GateValues["Z"] = 4
            return replace(
                FirstPlacement,
                Placed=replace(
                    FirstPlacement.Placed,
                    PlacedGates=[SimpleNamespace(**GateValues)],
                ),
            )
        ...
        def RouteBehavior(_X, _SuccessfulRoute, _Options):
            ...
            raise RoutingStageError(
                RoutingFailure(
                    Reason=RoutingFailureReason.RuntimeBudgetExceeded,
                    Stage="Candidate",
                    Detail="later candidate exhausted its adaptive slice",
                )
            )
        ...
        self.assertEqual(
            Failure.Diagnostics["ConflictFingerprint"],
            "higher-order-cut",
        )
        History = Failure.Diagnostics["AssignmentCutHistory"]
        self.assertEqual(len(History), 1)
        self.assertEqual(
            History[0]["ConflictGraph"]["CapacityOnePlannerWitness"],
            {
                "Owner": "Generate0",
                "Claimant": "Propagate1",
            },
    ...
    def testTranslatedPlacementIsNotRetainedAsAnotherCandidate(
        ...
        def ValidateIsolation(*_Arguments, **_Options) -> None:
            IsolationCalls.append(
                _Options["WorkCheck"].__name__
            )
        ...
        def RouteBehavior(X, _SuccessfulRoute, _Options):
            raise RoutingStageError(
                RoutingFailure(
                    Reason=(
                        RoutingFailureReason.TrackAssignmentConflict
                    ...
                    Stage="TrackAssignment",
                    Detail="force retained-candidate exhaustion",
                )
            )
        ...
        with (
            ...
            patch(
                "Compiler.Placement.Flow.PlacementAttempts.MeasureMandatoryAccessConflictProfile",
                wraps=MeasureMandatoryAccessConflictProfile,
            ) as MeasureAccess,
            self.assertRaises(RoutingStageError) as Context,
        ):
            self.RunTwoPlacementFlow(
                RouteBehavior=RouteBehavior,
                RouteOrder=RouteOrder,
                PlacementCallBehavior=PlaceBehavior,
                IsolationBehavior=ValidateIsolation,
            )

        self.assertEqual(RouteOrder, [0])
        self.assertEqual(
            IsolationCalls.count("CheckPlacementGeneration"),
            1,
        )
        self.assertEqual(
            IsolationCalls.count("CheckCandidateValidation"),
            1,
        )
        self.assertEqual(MeasureAccess.call_count, 1)
        Decisions = Context.exception.Failure.Diagnostics[
            "PlacementGenerationDecisions"
        ]
        Duplicates = [
            Decision
            for Decision in Decisions
            if Decision.get("Result") == "duplicate-placement"
        ]
        self.assertEqual(len(Duplicates), 1)
        Unique = [
            Decision
            for Decision in Decisions
            if Decision.get("Result") == "unique-placement"
        ]
        self.assertEqual(len(Unique), 1)
        self.assertFalse(
            Unique[0]["ExactStatePlacementEvaluationCacheHit"]
        )
        self.assertTrue(
            Duplicates[0]["ExactStatePlacementEvaluationCacheHit"]
        )
        self.assertNotEqual(
            Duplicates[0]["PlacementFingerprint"],
            Duplicates[0]["DuplicatePlacementFingerprint"],
        )
        self.assertTrue(
            Duplicates[0]["PlacementRetentionFingerprint"]
        )
