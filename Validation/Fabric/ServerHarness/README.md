...
```sh
gradle -p Validation/Fabric/ServerHarness build
```

Build output stays in `Cache/Gradle/ServerHarness/Build/`, and
`org.gradle.projectcachedir` retains the project cache at
`Cache/Gradle/ServerHarness/Project/`. Manager source is under
`Validation/Fabric/ServerManager/`.

The repository-owned local server runtime is
`Runtime/FabricServer/`. Place the Fabric 26.2 launcher at
`Runtime/FabricServer/fabric-server-launch.jar` and accept the Minecraft
...
`Runtime/FabricServer/mods/redstonecompiler-harness.jar` from the Gradle
